function [Ms] = opt_low_tfn_slack(G,n,nd,e,params,sparams)
...
i = 1;
T(:,i) = T0;
Qp(:,i) = params.cp*mdot_e(e.u_idx,idx).*(T(e.ui_idxnu,i)-params.TsetR)/10^3;
...
for i = 2:n.seg_T
    if mod((i-1)*params.dt_T,params.dt)==0
        ...
        Qp(:,i) = params.cp*mdot_e(e.u_idx,idx).*(T(e.ui_idxnu,i)-params.TsetR)/1000;   % kW
        % Ensure envelope is never exceeded
        intQ(:,i) = intQ(:,i-1)+(Qp(:,i)-Qb(:,idx))*params.dt_T;
...
opti_flow.solver('ipopt',struct('print_time',0),struct('print_level',0,'tol', 1e-2,'max_iter',1500))

Ms = opti_flow.to_function('Ms',{dP,T0,intQ0,Qb,Tamb,Ts,Cap_u,Cap_l,mdot_e,zeta_u,dP_e,mI,P_n,opti_flow.lam_g},...
    {dP_e,P_n,mdot_e,zeta_u,T,Qp,intQ,mI,opti_flow.lam_g,mdot_by,slack_Cap,cost_T,cost_Q,cost},...
    {'dP','T0','intQ0','Qb','Tamb','Ts','Cap_u','Cap_l','i_mdot_e','i_zeta_u','i_dP_e','i_mI','i_P_n','i_lam_g'},...
    {'dP_e','P_n','mdot_e','zeta_u','T','Qp','intQ','mI','lam_g','mdot_by','Cap_slack','cost_T','cost_Q','cost'});
